...
''';
  ...
  void initState() {
    ...
    for (int i = 0; i < _days.length; i++) {
      final day = DateTime(_days[i].year, _days[i].month, _days[i].day);
  ...
  Future<void> _addPlannedDate() async {
    ...
    if (selectedDate != null) {
      ...
      if (existing.isEmpty) {
        setState(() {
          _plannedDates.add(selectedDate);
          _plannedDates.sort();
        });
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Data aggiunta al piano')),
          );
        }
      } else {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Questa data è già nel piano')),
          );
        }
      }
    }
  ...
  void _showWeekPicker() async {
    ...
    if (selectedDate != null) {
      setState(() {
        _days = weekDays(selectedDate);
        // Find the selected day's index
  ...
  void _generateDailyMenu() async {
    ...
    try {
      ...
      if (response.containsKey('errore')) {
        if (mounted) {
          Navigator.pop(context);
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(response['errore'])),
          );
        }
        return;
      }
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        ...
        return Scaffold(
          appBar: AppBar(
            title: const Text('Il mio menù settimanale'),
          ),
          body: Column(
            children: [
              const SizedBox(height: 16),
